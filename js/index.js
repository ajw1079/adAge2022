...
//nike
//선택자 구성
const $nike_contCover = document.querySelector("#cont_01 .cont");
let $nike_contGroup = "";

//랜덤방식으로 0, 1, 2, 3 중에서 두개를 가져온다. (조건, 동일한 숫자가 존재하지 않음)
const nike_num_arr = [];
let nike_number;
...
//해당하는 배열데이터로부터 이미지를 화면상에 구현한다.
for(i=0; i<nike_num_arr.length; i++){
    $nike_contGroup += `<div 
    style="background-image:url(./img/nike_0${nike_num_arr[i]+1}.jpg)" 
    onclick = "location.href='./detail.html#nike_${nike_num_arr[i]}'"
    >
    </div>`;
}
...
//adidas
//선택자 구성
const $adidas_contCover = document.querySelector("#cont_02 .cont");
let $adidas_contGroup = "";

//랜덤방식으로 0, 1, 2, 3 중에서 두개를 가져온다. (조건, 동일한 숫자가 존재하지 않음)
const adidas_num_arr = [];
let adidas_number;
...
for(i=0; i<adidas_num_arr.length; i++){
    $adidas_contGroup += `<div 
    style="background-image:url(./img/adidas_0${adidas_num_arr[i]+1}.jpg)" 
    onclick = "location.href='./detail.html#adidas_${adidas_num_arr[i]}'"
    >
    </div>`;
}
...
//puma
//선택자 구성
const $puma_contCover = document.querySelector("#cont_03 .cont");
let $puma_contGroup = "";
...
for(i=0; i<puma_num_arr.length; i++){
    $puma_contGroup += `<div 
    style="background-image:url(./img/puma_0${puma_num_arr[i]+1}.jpg)" 
    onclick = "location.href='./detail.html#puma_${puma_num_arr[i]}'"
    >
    </div>`;
}
