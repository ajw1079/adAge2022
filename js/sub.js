...
const nikeTxt = "Just Do it - nike";
const adidasTxt = "Impossible is Nothing - adidas";
...
const hash_apply = () => {
    if($hash_txt){  //hash라는 정보가 존재한다면
        ...
        for(i = 0; i < 4; i++){  //i = 0 -> 1 -> 2 -> 3
            $contGroup += `<div 
            style="background-image:url(./img/${$hash_txt}_0${i+1}.jpg)" 
            onclick="location.href='./detail.html#${$hash_txt}_${i}'"></div>`;
        }
